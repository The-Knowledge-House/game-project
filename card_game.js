...
function buildDeck (){
  ...
  let  deck = [];
  //// i can use for loop but forEeach give to faster
  /* 
...
// 5. Done.
function greet(){
  let name = getInput(`welcome to the game!!!. What's your name?`);
    console.log(`Welcome ${name}, enjoy your game !\n`);
    return name;
...
function guess(cardOne, cardTwo){
  ...
  let input = getInput(`\n-  what Do you think is next card to be higher or lower than this card? \n- For higher, enter 'H' - For lower, enter 'L'`).toLowerCase();
  switch (input) {
    case 'h':
    ...
    case 'l':
      ...
      break;
    default:
      console.log('\n Please guess either H or L next time! \nNo points earned this round.\n');
  }
...
if (deck.length === 0){
  console.log(`You have lost . You You're out of cards.`)
