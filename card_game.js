...
// the returning value of getInput() is a string of whatever the user has typed as
//the response
...
// 6. inside your inner for loop, push your looped iterations of ranks and suits as
//OBJECTS into the empty deck array.Add a third property to this object with the key "value"
//and the value equal to the current iterator.
...
function buildDeck() {
  let suits = ["clubs", "diamonds", "hearts", "shades"];
  let ranks = [
    "A",
    "2",
    "3",
  ...
  for (let i = 0; i < ranks.length; i++) {
    for (let j = 0; j < suits.length; j++) {
      deck.push({ suits: suits[j], rank: ranks[i], value: i });
    }
  }
  return deck;
}
...
// 1. use a function declaration to create a function called shuffle that
//takes deck as an argument.

// 2. Inside this function create a variable called "shuffledDeck" that takes deck
//as its value.

// 3. Using "let" declare three new variables: currentIndex, whose value should equal
//the length of the deck array, and two more: temporaryValue and randomIndex,
//each of which should currently have no value assigned.

// 4. Create a while loop whose condition is that "currentIndex" does not equal 0,
//so that we stop looping once we've gone through all 52 cards.

// 5. Inside the while loop, use the javascript Math.methods to generate a
//random integer between 0 and "currentIndex"

// 6. Inside the while loop, decrement current index by 1. (should be after step 9)

// 7. Inside the while loop, assign "temporaryValue" with "shuffledDeck"
//(which is an array) to the[currentIndex].

// 8. Still inside, assign "shuffledDeck[currentIndex]" a value of
//shuffledDeck to the[randomIndex]

// 9. Still inside, assign "shuffledDeck[randomIndex]" a value of
//"temporaryValue".  (currentIndex //i--;)
...
function shuffle(deck) {
  ...
  while (currentIndex !== 0) {
    ...
    temporaryValue = shuffledDeck[currentIndex]; //7- Taking last card from stack (index-51)
    shuffledDeck[currentIndex] = shuffledDeck[randomIndex]; //8- Getting a random card from stack (index-?) and switch with last card (index-51)
    shuffledDeck[randomIndex] = temporaryValue; //9 Take last card (index-51) and switch where random card was at (index-?)
...
// 3. declare a variable called "input" that uses getInput() to ask the user
//if they think the next card will be higher(h) or lower(l) than their current
//card and stores the user's response.
// 4. use a conditional statement to see if "input" equals "h" or "l".
// 5. If input equals h, return an expression that checks if the outcome of the
//compare function (using the same arguments as you used for guess) is a negative number.
// 6. If input equals l, check and see if it's a positive number.
// 7. If input doesn't equal h or l, tell the user that they need to guess either
...
//needs to take the results one of our other functions as its argument...
// 3. declare a variable called playerName that takes the result of the greet
//function as its value.
// 4. using let, declare a score variable that's currently set to the number zero
// 5. use an array method on deck to remove the last object in deck. using let,
//declare a variable called currentCard and assign it this value.
// 6. create a while loop whose conditions are that score is less than five AND less
//than the amount of items still in the deck array.
// 7. Inside the while loop, use an array method on deck to remove the last
//object and assign that value to a variable named nextCard.
// 8. Inside the while loop, create a conditional statement. If the outcome of
//guess is true, increment the score by 1, congratulate the user, and tell them their
//score.If it's false, tell them they were wrong and got no points.
// 9. Close the conditional statement and assign nextCard to currentCard. You may have
//to write this as the type of variable that's always global...
// 10. Close the while loop and use a ternary statement that checks if the length of
//the deck array has reached zero. If it has not, tell the user that they won.If it has
//reached zero, tell them that they're out of cards and they lost.
