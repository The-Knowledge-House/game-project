...
function buildDeck() {
	...
	const suits = [ 'Heart', 'Clovers', 'Diamonds', 'Clubs' ];
	...
	for (let i = 0; i < suits.length; i++) {
		for (let j = 0; j < ranks.length; j++) {
			// 6. inside your inner for loop, push your looped iterations of ranks and suits as OBJECTS into the empty deck array.
			//Add a third property to this object with the key "value" and the value equal to the current iterator.
			// HINT: The result of step 6 is that each card will be an object inside of the deck array, for example
			//[{suit: "diamonds", rank: "A", value: 0}, {suit: "diamonds", rank: "2", value: 1},...{etc}]. For example, if we wanted to organize
			//the players and teams of the NBA with index numbers, we could write: nba.push({player: players[i], team: teams[n], index: i})
			deck.push({ suit: suits[i], rank: ranks[j], value: j });
		}
	}
	return deck;
}
...
greet = () => {
	...
	let name = getInput('Welcome player, What is your name?');
	// 3. Console.log name
	console.log(name);
	// 4. return name
	return name;
...
guess = (card1, card2) => {
	...
	// 3. declare a variable called "input" that uses getInput() to ask the user if they think the next card will be higher (h) or
	//lower(l) than their current card and stores the user's response.
	let input = getInput('Do you think the next card will be higher(h) or lower(l) than the current card?');
	...
	if (input === 'h') {
		...
		console.log('You need to enter either h or l, you will recieve no points');
		return false;
	}
};
// 5. If input equals h, return an expression that checks if the outcome of the compare function (using the same arguments as
//you used for guess) is a negative number.
// 6. If input equals l, check and see if it's a positive number.
// 7. If input doesn't equal h or l, tell the user that they need to guess either h or l and that they get no points for this round,
//then return false.
