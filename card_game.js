...
// 5. inside the buildDeck function, create a for loop INSIDE of another for loop. The outer loop should loop through the ranks. 
//    The inner loop should loop through the suits. Make sure to use different variables for your iterators.
...
//    HINT: The result of step 6 is that each card will be an object inside of the deck array, 
//    for example [{suit: "diamonds", rank: "A", value: 0}, {suit: "diamonds", rank: "2", value: 1},...{etc}]. 
//    For example, if we wanted to organize the players and teams of the NBA with index numbers, 
//    we could write: nba.push({player: players[i], team: teams[n], index: i})
...
function buildDeck(){
  ...
  for(let r = 0; r <= ranks.length; r++) {
    for(let s = 0; s <= suits.length; s++) {
      deck.push({ suits: suits[s], rank: ranks[r], value: r});
    }
  }
  return deck;
}
...
// 3. Using "let" declare three new variables: currentIndex, whos value should equal the length of the deck array, 
//and two more: temporaryValue and randomIndex, each of which should currently have no value assigned.
// 4. Create a while loop whos condition is that "currentIndex" does not equal 0, so that we stop looping once we've gone 
...
function shuffle(deck){
  ...
  while (currentIndex !== 0) {
    ...
    temporaryValue = shuffledDeck[currentIndex]; // 7. Saving the index card to a temporary variable
    shuffledDeck[currentIndex] = shuffledDeck[randomIndex]; // 8. Randominzing the cards from the deck
    shuffledDeck[randomIndex] = temporaryValue; // 9. Stores a new card to a temporary variable
    
    currentIndex --; // 6. Decrement index by 1
  }
  return shuffledDeck;
}
...
function greet() {
  let name = getInput("Hello there stranger! What's your first name?");
  console.log(name);
  return name;
}
...
// 3. declare a variable called "input" that uses getInput() to ask the user if they think the next card will be higher 
//(h) or lower (l) than their current card and stores the user's response.
// 4. use a conditional statement to see if "input" equals "h" or "l".
// 5. If input equals h, return an expression that checks if the outcome of the compare function 
//(using the same arguments as you used for guess) is a negative number.
// 6. If input equals l, check and see if it's a positive number.
// 7. If input doesn't equal h or l, tell the user that they need to guess either h or l 
//and that they get no points for this round, then return false.

function guess(firstCard, secondCard) {
  console.log(
    `Your first card is a ${firstCard.ranks} of ${firstCard.suits}.  Your second card is a ${secondCard.ranks} of ${secondCard.suits}.`
  );
  ...
  if (input == "h") {
    ...
    console.log(`You need to guess either h or l. Sorry, you get no points for this round!`);
    return false;
  }
}
...
// 2. declare a variable called deck (it's okay to reuse -- remember scope!) that takes the result of the shuffle function. 
...
// 7. Inside the while loop, use an array method on deck to remove the last object and assign that value to a variable 
//named nextCard.
// 8. Inside the while loop, create a conditional statement. If the outcome of guess is true, increment the score by 1, 
//congratulate the user, and tell them their score. If it's false, tell them they were wrong and got no points.
// 9. Close the conditional statement and assign nextCard to currentCard. You may have to write this as the type of 
//variable that's always global...
...
function playGame() {
  ...
  while (score < 5 && score < deck.length) {
    ...
    if(guess(currentCard, nextCard) == true) {
      ...
      console.log(`Aw snap, you get no points`);
    }
    currentCard = nextCard;
  }
  deck.legnth === 0
    ? console.log(`Sorry you're out of cards. You lost!`)
    : `Hip Hop Hooray! You won!`;
}
