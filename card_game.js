...
// 5. inside the buildDeck function, create a for loop INSIDE of another for loop. The outer loop should loop through the ranks. The inner loop should loop through the suits. Make sure to use different letiables for your iterators.
...
// 2. Inside this function create a letiable called "shuffledDeck" that takes deck as its value.
// 3. Using "let" declare three new letiables: currentIndex, whos value should equal the length of the deck array, and two more: temporaryValue and randomIndex, each of which should currently have no value assigned.
...
// 2. Inside that function, declare a letiable called "name" and use "getInput()" to welcome the user to the game, ask for their name, and assign their answer.
// 3. Console.log name
// 4. return name
// 5. Done.

function greet() {
  let name = getInput("Welcome, what is your name?");
  console.log(name);
  return name;
}
...
// 3. declare a letiable called "input" that uses getInput() to ask the user if they think the next card will be higher (h) or lower (l) than their current card and stores the user's response.
...
function guess(cardOne, cardTwo) {
  ...
  let input = getInput("Will your next card be higher (h) or lower (l) than your current card?");
  if (input == "h") {
...
// 2. declare a letiable called deck (it's okay to reuse -- remember scope!) that takes the result of the shuffle function. Remember that the shuffle function needs to take the results one of our other functions as its argument...
// 3. declare a letiable called playerName that takes the result of the greet function as its value.
// 4. using let, declare a score letiable that's currently set to the number zero
// 5. use an array method on deck to remove the last object in deck. using let, declare a letiable called currentCard and assign it this value.
...
// 7. Inside the while loop, use an array method on deck to remove the last object and assign that value to a letiable named nextCard.
...
// 9. Close the conditional statement and assign nextCard to currentCard. You may have to write this as the type of letiable that's always global...
...
function playGame() {
  ...
  if (deck.length === 0) {
    console.log(`You lost! You are out of cards.`)
  } else {
    console.log('You won!');
  }
}
