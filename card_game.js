...
function shuffle(deck) {
	...
	while (currentIndex !== 0) {
		...
		// storing current index of shuffle deck inside temporaryValue
		temporaryValue = shuffleDeck[currentIndex];
		// set to random object using shuffleDeck[randomIndex]
		shuffleDeck[currentIndex] = shuffleDeck[randomIndex];
		// then assign it to temporaryValue
...
function greet() {
	...
	console.log(`Welcome to the High or Low card game ${name}`);
	return name;
}
...
function guess(card1, card2) {
	...
	let input = getInput(
		`Do you think the next card will be higher (h) or (l) than your current card`,
	);
	...
	if (input === "h") {
		...
		console.log(
			`you will need to guess (h) or (l), you get no points this round`,
		);
		return false;
	}
}
...
function playGame() {
	...
	while (score < 5 && score < deck.length) {
		...
		if (guess(currentCard, nextCard) === true) {
			...
			console.log(`You were wrong ${playerName}, you get no points`);
		}
		currentCard = nextCard;
	}
	deck.length === 0
		? `${playerName} you are out of cards and you have lost the game`
		: `${playerName} you have won the game`;
}
playGame();
